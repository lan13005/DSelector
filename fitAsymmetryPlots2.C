...
void fitAsymmetryPlots2(){
	...
	static const int num_baryonBins=3;
	...
	cout << "Shape of the tensor of histograms: ["<<nTagEta<<"]["<<num_baryonBins<<"]["<<nDataSets<<"]["<<nSetsBS<<"]"<< endl;
	TH1F *phi000_eta_unscaled[nTagEta][num_baryonBins][nDataSets][nSetsBS]; // 2 for the tag and 3 for the datasets
	TH1F *phi045_eta_unscaled[nTagEta][num_baryonBins][nDataSets][nSetsBS]; 
	TH1F *phi090_eta_unscaled[nTagEta][num_baryonBins][nDataSets][nSetsBS];
	TH1F *phi135_eta_unscaled[nTagEta][num_baryonBins][nDataSets][nSetsBS];
	TH1F *phiAMO_eta_unscaled[nTagEta][num_baryonBins][nDataSets][nSetsBS];
	TH1F *phi000_pi0_unscaled[nTagEta][num_baryonBins][nDataSets][nSetsBS];
	TH1F *phi045_pi0_unscaled[nTagEta][num_baryonBins][nDataSets][nSetsBS];
	TH1F *phi090_pi0_unscaled[nTagEta][num_baryonBins][nDataSets][nSetsBS];
	TH1F *phi135_pi0_unscaled[nTagEta][num_baryonBins][nDataSets][nSetsBS];
	TH1F *phiAMO_pi0_unscaled[nTagEta][num_baryonBins][nDataSets][nSetsBS];
	// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if (nSetsBS == 1){
		for (int iSet=0; iSet<nSetsBS; ++iSet){ // we add 1 since using the full dataset takes the first element of the array
			for (int iData=0; iData <nDataSets; ++iData){
				...
				for (int iTag=0; iTag < nTagEta; ++iTag){
					for (int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){
						string affix = "Bin"+to_string(iBaryonBin);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_045_Mpi0p"+affix).c_str(), phi045_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_045_Metap"+affix).c_str(), phi045_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_090_Mpi0p"+affix).c_str(), phi090_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_090_Metap"+affix).c_str(), phi090_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_AMO_Mpi0p"+affix).c_str(), phiAMO_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_AMO_Metap"+affix).c_str(), phiAMO_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
						// Need to scale these para yields by flux ratio.
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_000_Mpi0p"+affix).c_str(), phi000_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
						...
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_000_Metap"+affix).c_str(), phi000_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_135_Mpi0p"+affix).c_str(), phi135_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_135_Metap"+affix).c_str(), phi135_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
					}
		...
		std::vector<double> values[nTagEta][num_baryonBins][nDataSets][nHists];
		std::vector<double> weights[nTagEta][num_baryonBins][nDataSets][nHists];
		...
		for ( int iData=0; iData<nDataSets; ++iData){ // the dataset we use, 2017 or the two 2018 sets
			for (int iTag=0; iTag < nTagEta; ++iTag){ // nothing or backwardPi0/Etap
				for (int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){ // t bins
					string name0 = "prodPlanePSphi"+tagEta[iTag]+"_045_Mpi0pBin"+to_string(iBaryonBin);
					string name1 = "prodPlanePSphi"+tagPi0[iTag]+"_045_MetapBin"+to_string(iBaryonBin);
					string name2 = "prodPlanePSphi"+tagEta[iTag]+"_090_Mpi0pBin"+to_string(iBaryonBin);
					string name3 = "prodPlanePSphi"+tagPi0[iTag]+"_090_MetapBin"+to_string(iBaryonBin);
					string name4 = "prodPlanePSphi"+tagEta[iTag]+"_AMO_Mpi0pBin"+to_string(iBaryonBin);
					string name5 = "prodPlanePSphi"+tagPi0[iTag]+"_AMO_MetapBin"+to_string(iBaryonBin);
					string name6 = "prodPlanePSphi"+tagEta[iTag]+"_000_Mpi0pBin"+to_string(iBaryonBin);
					string name7 = "prodPlanePSphi"+tagPi0[iTag]+"_000_MetapBin"+to_string(iBaryonBin);
					string name8 = "prodPlanePSphi"+tagEta[iTag]+"_135_Mpi0pBin"+to_string(iBaryonBin);
					string name9 = "prodPlanePSphi"+tagPi0[iTag]+"_135_MetapBin"+to_string(iBaryonBin);
					string nameTag = "_iData"+to_string(iData)+"_iSet0"; 

        				phi045_eta_unscaled[iTag][iBaryonBin][iData][0] = new TH1F((name0+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi045_pi0_unscaled[iTag][iBaryonBin][iData][0] = new TH1F((name1+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi090_eta_unscaled[iTag][iBaryonBin][iData][0] = new TH1F((name2+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi090_pi0_unscaled[iTag][iBaryonBin][iData][0] = new TH1F((name3+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phiAMO_eta_unscaled[iTag][iBaryonBin][iData][0] = new TH1F((name4+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phiAMO_pi0_unscaled[iTag][iBaryonBin][iData][0] = new TH1F((name5+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi000_eta_unscaled[iTag][iBaryonBin][iData][0] = new TH1F((name6+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi000_pi0_unscaled[iTag][iBaryonBin][iData][0] = new TH1F((name7+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi135_eta_unscaled[iTag][iBaryonBin][iData][0] = new TH1F((name8+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi135_pi0_unscaled[iTag][iBaryonBin][iData][0] = new TH1F((name9+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
					...
					std::vector<TH1F*> hists = {phi045_eta_unscaled[iTag][iBaryonBin][iData][0],phi045_pi0_unscaled[iTag][iBaryonBin][iData][0]
						,phi090_eta_unscaled[iTag][iBaryonBin][iData][0]
						,phi090_pi0_unscaled[iTag][iBaryonBin][iData][0],phiAMO_eta_unscaled[iTag][iBaryonBin][iData][0],phiAMO_pi0_unscaled[iTag][iBaryonBin][iData][0]
						,phi000_eta_unscaled[iTag][iBaryonBin][iData][0],phi000_pi0_unscaled[iTag][iBaryonBin][iData][0],phi135_eta_unscaled[iTag][iBaryonBin][iData][0]
						,phi135_pi0_unscaled[iTag][iBaryonBin][iData][0]};
					...
					for ( int iHist=0; iHist<nHists; ++iHist){
						while (*(inFiles[iHist]) >> value >> weight){
							//cout << value << endl;
							values[iTag][iBaryonBin][iData][iHist].push_back(value);
							weights[iTag][iBaryonBin][iData][iHist].push_back(weight);
							hists[iHist]->Fill(value,weight);
						}
						hists[iHist]->Print();
						nevents[iHist] = values[iTag][iBaryonBin][iData][iHist].size();	
					...
					for (int iSet=1; iSet<nSetsBS; ++iSet){ // we add 1 since using the full dataset takes the first element of the array
						...
						string cutTag = "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees";
        					phi045_eta_unscaled[iTag][iBaryonBin][iData][iSet] = new TH1F((name0+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi045_pi0_unscaled[iTag][iBaryonBin][iData][iSet] = new TH1F((name1+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi090_eta_unscaled[iTag][iBaryonBin][iData][iSet] = new TH1F((name2+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi090_pi0_unscaled[iTag][iBaryonBin][iData][iSet] = new TH1F((name3+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phiAMO_eta_unscaled[iTag][iBaryonBin][iData][iSet] = new TH1F((name4+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phiAMO_pi0_unscaled[iTag][iBaryonBin][iData][iSet] = new TH1F((name5+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi000_eta_unscaled[iTag][iBaryonBin][iData][iSet] = new TH1F((name6+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi000_pi0_unscaled[iTag][iBaryonBin][iData][iSet] = new TH1F((name7+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi135_eta_unscaled[iTag][iBaryonBin][iData][iSet] = new TH1F((name8+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi135_pi0_unscaled[iTag][iBaryonBin][iData][iSet] = new TH1F((name9+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
						hists = {phi045_eta_unscaled[iTag][iBaryonBin][iData][iSet],phi045_pi0_unscaled[iTag][iBaryonBin][iData][iSet],phi090_eta_unscaled[iTag][iBaryonBin][iData][iSet]
							,phi090_pi0_unscaled[iTag][iBaryonBin][iData][iSet],phiAMO_eta_unscaled[iTag][iBaryonBin][iData][iSet],phiAMO_pi0_unscaled[iTag][iBaryonBin][iData][iSet]
							,phi000_eta_unscaled[iTag][iBaryonBin][iData][iSet],phi000_pi0_unscaled[iTag][iBaryonBin][iData][iSet],phi135_eta_unscaled[iTag][iBaryonBin][iData][iSet]
							,phi135_pi0_unscaled[iTag][iBaryonBin][iData][iSet]};
						int randIdx;
						for ( int iHist=0; iHist<nHists; ++iHist){
							for (int ievent=0; ievent<nevents[iHist]; ++ievent){
								randIdx = rand() % nevents[iHist];		
								value = values[iTag][iBaryonBin][iData][iHist][randIdx];
								weight = weights[iTag][iBaryonBin][iData][iHist][randIdx];
								hists[iHist]->Fill(value, weight);
							}
						}
					}
	...
	// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	// --------------------------------------------------------------------------------------------------------------------
	double asymmetries_000_eta[nTagEta][nSetsBS][num_baryonBins];
	double asymmetries_045_eta[nTagEta][nSetsBS][num_baryonBins];
	double asymmetries_000_eta_err[nTagEta][nSetsBS][num_baryonBins];
	double asymmetries_045_eta_err[nTagEta][nSetsBS][num_baryonBins];
	double asymmetries_000_pi0[nTagEta][nSetsBS][num_baryonBins];
	double asymmetries_045_pi0[nTagEta][nSetsBS][num_baryonBins];
	double asymmetries_000_pi0_err[nTagEta][nSetsBS][num_baryonBins];
	double asymmetries_045_pi0_err[nTagEta][nSetsBS][num_baryonBins];
	...
	for (int iSet=0; iSet <nSetsBS; ++iSet) {
		...
		TH1F *phi000_eta_total[nTagEta][num_baryonBins];
		TH1F *phi045_eta_total[nTagEta][num_baryonBins];
		TH1F *phi090_eta_total[nTagEta][num_baryonBins];
		TH1F *phi135_eta_total[nTagEta][num_baryonBins];
		TH1F *phiAMO_eta_total[nTagEta][num_baryonBins];
		TH1F *phi000_pi0_total[nTagEta][num_baryonBins];
		TH1F *phi045_pi0_total[nTagEta][num_baryonBins];
		TH1F *phi090_pi0_total[nTagEta][num_baryonBins];
		TH1F *phi135_pi0_total[nTagEta][num_baryonBins];
		TH1F *phiAMO_pi0_total[nTagEta][num_baryonBins];
		...
		for (int iTag=0; iTag < nTagEta; ++iTag) {
			for (int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){
				...
				phi000_eta_total[iTag][iBaryonBin] = (TH1F *)phi000_eta_unscaled[iTag][iBaryonBin][0][iSet]->Clone();
				phi000_pi0_total[iTag][iBaryonBin] = (TH1F *)phi000_pi0_unscaled[iTag][iBaryonBin][0][iSet]->Clone();
				phi135_eta_total[iTag][iBaryonBin] = (TH1F *)phi135_eta_unscaled[iTag][iBaryonBin][0][iSet]->Clone();
				phi135_pi0_total[iTag][iBaryonBin] = (TH1F *)phi135_pi0_unscaled[iTag][iBaryonBin][0][iSet]->Clone();
				maximum_before = phi000_eta_total[iTag][iBaryonBin]->GetMaximum();
				cout << "-Maximum before scaling of phi000_eta_total[" << iTag << "][" << iBaryonBin << "]: " << maximum_before << endl;
				cout << "-NEntries before scaling of phi000_eta_total[" << iTag << "]" << iBaryonBin << "]: " << phi000_eta_total[iTag][iBaryonBin]->GetEntries() << endl;
				phi000_eta_total[iTag][iBaryonBin]->Scale(fluxRatios_90_0[0]);
				phi000_pi0_total[iTag][iBaryonBin]->Scale(fluxRatios_90_0[0]);
				phi135_eta_total[iTag][iBaryonBin]->Scale(fluxRatios_45_135[0]);
				phi135_pi0_total[iTag][iBaryonBin]->Scale(fluxRatios_45_135[0]);
				maximum_after = phi000_eta_total[iTag][iBaryonBin]->GetMaximum();
				cout << "-Maximum after scaling of phi000_eta_total[" << iTag << "]" << iBaryonBin << "]: " << maximum_after << endl;
				cout << "-NEntries after scaling of phi000_eta_total[" << iTag << "]" << iBaryonBin << "]: " << phi000_eta_total[iTag][iBaryonBin]->GetEntries() << endl; 
				cout << "-Scale factor vs Expected: " << maximum_after/maximum_before << " vs " << fluxRatios_90_0[0] << endl; 
				phi045_eta_total[iTag][iBaryonBin] = (TH1F *)phi045_eta_unscaled[iTag][iBaryonBin][0][iSet]->Clone();
				phi045_pi0_total[iTag][iBaryonBin] = (TH1F *)phi045_pi0_unscaled[iTag][iBaryonBin][0][iSet]->Clone();
				phi090_eta_total[iTag][iBaryonBin] = (TH1F *)phi090_eta_unscaled[iTag][iBaryonBin][0][iSet]->Clone();
				phi090_pi0_total[iTag][iBaryonBin] = (TH1F *)phi090_pi0_unscaled[iTag][iBaryonBin][0][iSet]->Clone();
				phiAMO_eta_total[iTag][iBaryonBin] = (TH1F *)phiAMO_eta_unscaled[iTag][iBaryonBin][0][iSet]->Clone();
				phiAMO_pi0_total[iTag][iBaryonBin] = (TH1F *)phiAMO_pi0_unscaled[iTag][iBaryonBin][0][iSet]->Clone();

				totalEntriesInPhi000_eta_total += phi000_eta_total[iTag][iBaryonBin]->GetEntries();
				...
				for (int iData=1; iData < nDataSets; ++iData){
					...
					totalEntriesInPhi000_eta_total += phi000_eta_unscaled[iTag][iBaryonBin][iData][iSet]->GetEntries();
					...
					phi000_eta_unscaled[iTag][iBaryonBin][iData][iSet]->Scale(fluxRatios_90_0[iData]);
					phi000_pi0_unscaled[iTag][iBaryonBin][iData][iSet]->Scale(fluxRatios_90_0[iData]);
					phi135_eta_unscaled[iTag][iBaryonBin][iData][iSet]->Scale(fluxRatios_45_135[iData]);
					phi135_pi0_unscaled[iTag][iBaryonBin][iData][iSet]->Scale(fluxRatios_45_135[iData]);
					phi000_eta_total[iTag][iBaryonBin]->Add(phi000_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
        	        		phi000_pi0_total[iTag][iBaryonBin]->Add(phi000_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
        	        		phi135_eta_total[iTag][iBaryonBin]->Add(phi135_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
        	        		phi135_pi0_total[iTag][iBaryonBin]->Add(phi135_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);

					// Dont have to scale these guys so just add them
        	        		phi045_eta_total[iTag][iBaryonBin]->Add(phi045_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
        	        		phi045_pi0_total[iTag][iBaryonBin]->Add(phi045_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
        	        		phi090_eta_total[iTag][iBaryonBin]->Add(phi090_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
        	        		phi090_pi0_total[iTag][iBaryonBin]->Add(phi090_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
        	        		phiAMO_eta_total[iTag][iBaryonBin]->Add(phiAMO_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
        	        		phiAMO_pi0_total[iTag][iBaryonBin]->Add(phiAMO_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
				}
			}
		...
		double xBinsEta[num_baryonBins];
		double xBinsEta_err[num_baryonBins];
		double xBinsPi0[num_baryonBins];
		double xBinsPi0_err[num_baryonBins];
		double xBinSize=0.4;

		Int_t fitStatus;
		for (int iTag=0; iTag<nTagEta; ++iTag){
			for (int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){ //num_baryonBins
				...
				TH1F *phi000_eta = phi000_eta_total[iTag][iBaryonBin];
				TH1F *phi045_eta = phi045_eta_total[iTag][iBaryonBin];
				TH1F *phi090_eta = phi090_eta_total[iTag][iBaryonBin];
				TH1F *phi135_eta = phi135_eta_total[iTag][iBaryonBin];
				TH1F *phiAMO_eta = phiAMO_eta_total[iTag][iBaryonBin];
				TH1F *phi000_pi0 = phi000_pi0_total[iTag][iBaryonBin];
				TH1F *phi045_pi0 = phi045_pi0_total[iTag][iBaryonBin];
				TH1F *phi090_pi0 = phi090_pi0_total[iTag][iBaryonBin];
				TH1F *phi135_pi0 = phi135_pi0_total[iTag][iBaryonBin];
				TH1F *phiAMO_pi0 = phiAMO_pi0_total[iTag][iBaryonBin];
				...
				// *****************************
				xBinsEta[iBaryonBin] = 1.5+(iBaryonBin+0.5)*xBinSize;
				xBinsEta_err[iBaryonBin] = xBinSize/2;
				xBinsPi0[iBaryonBin] = 1.4+(iBaryonBin+0.5)*xBinSize;
				xBinsPi0_err[iBaryonBin] = xBinSize/2;
				...
				asymmetries_000_eta[iTag][iSet][iBaryonBin] = fit_asym->GetParameter(2);
				asymmetries_000_eta_err[iTag][iSet][iBaryonBin] = fit_asym->GetParError(2);
				...
				asymmetries_045_eta[iTag][iSet][iBaryonBin] = fit_asym->GetParameter(2);
				asymmetries_045_eta_err[iTag][iSet][iBaryonBin] = fit_asym->GetParError(2);
				...
				if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/asymmetry"+tagEta[iTag]+"_Mpi0pBin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".png").c_str()); }
				
				//allCanvases->Clear();
				//allCanvases->Divide(2,1);
				//allCanvases->cd(1);
				//likelihoodFit_000_090_eta->Draw("ALP");
				//likelihoodFit_000_090_eta->SetTitle(("0/90 - tBin"+to_string(iBaryonBin)).c_str());
				//allCanvases->cd(2);
				//likelihoodFit_045_135_eta->Draw("ALP");
				//likelihoodFit_045_135_eta->SetTitle(("045_135 - tBin"+to_string(iBaryonBin)).c_str());
				//if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/likelihood"+tagEta[iTag]+"_Mpi0etaBin"+to_string(iMass)+"_Mpi0pBin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".root").c_str()); }
				...
				asymmetries_000_pi0[iTag][iSet][iBaryonBin] = fit_asym->GetParameter(2);
				asymmetries_000_pi0_err[iTag][iSet][iBaryonBin] = fit_asym->GetParError(2);
				...
				asymmetries_045_pi0[iTag][iSet][iBaryonBin] = fit_asym->GetParameter(2);
				asymmetries_045_pi0_err[iTag][iSet][iBaryonBin] = fit_asym->GetParError(2);
				TGraph* likelihoodFit_045_135_pi0 = new TGraph(50); 
				//fitPointer->Scan(2,likelihoodFit_045_135_pi0,-2,2);

				if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/asymmetry"+tagPi0[iTag]+"_MetapBin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".png").c_str()); }
				...
				//allCanvases->cd(2);
				//likelihoodFit_045_135_eta->Draw("ALP");
				//likelihoodFit_045_135_eta->SetTitle(("045/135 - tBin"+to_string(iBaryonBin)).c_str());
				//if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/likelihood"+tagEta[iTag]+"_Mpi0etaBin"+to_string(iMass)+"_Mpi0pBin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".root").c_str()); }
				...
				if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/phiYieldFits"+tagEta[iTag]+"_Mpi0pBin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".png").c_str()); }
				...
				if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/phiYieldFits"+tagPi0[iTag]+"_MetapBin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".png").c_str()); }
			...
			allCanvases->Divide(2,1);
			allCanvases->cd(1);
			auto gr_000 = new TGraphErrors(num_baryonBins,xBinsEta,asymmetries_000_eta[iTag][iSet],xBinsEta_err,asymmetries_000_eta_err[iTag][iSet]);
			...
			allCanvases->cd(2);
			auto gr_045 = new TGraphErrors(num_baryonBins,xBinsPi0,asymmetries_000_pi0[iTag][iSet],xBinsPi0_err,asymmetries_000_pi0_err[iTag][iSet]);
			...
			//	gr_etas[counter] = new TGraphErrors(num_baryonBins,tBins,&(phis_eta_PSig[counter][0]),tBins_err,&(phis_eta_PSig_err[counter][0]));
			...
			//	gr_pi0s[counter] = new TGraphErrors(num_baryonBins,tBins,&(phis_pi0_PSig[counter][0]),tBins_err,&(phis_pi0_PSig_err[counter][0]));
			...
			//allCanvases->cd(2);
   			//legend_pi0->Draw();
			//if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/PSigma_iSet"+to_string(iSet)+".png").c_str()); }
		}
	...
	if (nSetsBS != 1) {
		...
		double asymmetries_000_eta_err_BS[nTagEta][num_baryonBins][nSetsJustBS]; // nSetsJustBS is the index with values = the running population std
		double asymmetries_045_eta_err_BS[nTagEta][num_baryonBins][nSetsJustBS];
		double asymmetries_000_pi0_err_BS[nTagEta][num_baryonBins][nSetsJustBS];
		double asymmetries_045_pi0_err_BS[nTagEta][num_baryonBins][nSetsJustBS];
		...
		for (int iTag=0; iTag < 1; ++iTag) { //nTagEta
			...
			for (int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){
				asymmetries_000_eta_err_BS[iTag][iBaryonBin][0] = 0;
				asymmetries_045_eta_err_BS[iTag][iBaryonBin][0] = 0;
				asymmetries_000_pi0_err_BS[iTag][iBaryonBin][0] = 0;
				asymmetries_045_pi0_err_BS[iTag][iBaryonBin][0] = 0;
				nBoostdstrapped[0] = 1;
			...
			allAsymsBSFile << "asymmetries_000_eta[iTag][iSet][iBaryonBin]" << endl;
			for(int iSetJustBS=3; iSetJustBS<nSetsBS+2; ++iSetJustBS){
				...
				for(int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){
					...
					for ( int iSet=1; iSet < iSetJustBS; ++iSet ){
						mean_000_eta += asymmetries_000_eta[iTag][iSet][iBaryonBin]; // these asymmetries begin with the "full" fits. Only do BS for lowest Mpi0eta threshold
						mean_045_eta += asymmetries_045_eta[iTag][iSet][iBaryonBin];
						mean_000_pi0 += asymmetries_000_pi0[iTag][iSet][iBaryonBin];
						mean_045_pi0 += asymmetries_045_pi0[iTag][iSet][iBaryonBin];
					}
					...
					for ( int iSet=1; iSet<iSetJustBS; ++iSet ){ //again we start at 1 since "full" uses the 0 position
						summedSqDiff_000_eta += TMath::Power(asymmetries_000_eta[iTag][iSet][iBaryonBin]-mean_000_eta,2);
						summedSqDiff_045_eta += TMath::Power(asymmetries_045_eta[iTag][iSet][iBaryonBin]-mean_045_eta,2);
						summedSqDiff_000_pi0 += TMath::Power(asymmetries_000_pi0[iTag][iSet][iBaryonBin]-mean_000_pi0,2);
						summedSqDiff_045_pi0 += TMath::Power(asymmetries_045_pi0[iTag][iSet][iBaryonBin]-mean_045_pi0,2);
					}
					...
					summedSqDiff_000_eta /= asymmetries_000_eta_err[iTag][0][iBaryonBin];
					summedSqDiff_045_eta /= (iSetJustBS-2);
					summedSqDiff_045_eta = sqrt(summedSqDiff_045_eta);
					summedSqDiff_045_eta /= asymmetries_045_eta_err[iTag][0][iBaryonBin]; 
					summedSqDiff_000_pi0 /= (iSetJustBS-2);
					summedSqDiff_000_pi0 = sqrt(summedSqDiff_000_pi0);
					summedSqDiff_000_pi0 /= asymmetries_000_pi0_err[iTag][0][iBaryonBin]; 
					summedSqDiff_045_pi0 /= (iSetJustBS-2);
					summedSqDiff_045_pi0 = sqrt(summedSqDiff_045_pi0);
					summedSqDiff_045_pi0 /= asymmetries_045_pi0_err[iTag][0][iBaryonBin]; 

					asymmetries_000_eta_err_BS[iTag][iBaryonBin][iSetJustBS-2] = summedSqDiff_000_eta;
					asymmetries_045_eta_err_BS[iTag][iBaryonBin][iSetJustBS-2] = summedSqDiff_045_eta;
					asymmetries_000_pi0_err_BS[iTag][iBaryonBin][iSetJustBS-2] = summedSqDiff_000_pi0;
					asymmetries_045_pi0_err_BS[iTag][iBaryonBin][iSetJustBS-2] = summedSqDiff_045_pi0;
					cout << "(iBaryonBin, iSetJustBS)=(" << iBaryonBin << ", " << iSetJustBS << ") currentSTD:" << summedSqDiff_000_eta << endl;
					allAsymsBSFile << "asymmetries_000_eta[" << iTag << "]["<< iSetJustBS-2 << "][" << iBaryonBin << "] = " << asymmetries_000_eta[iTag][iSetJustBS-2][iBaryonBin] << endl;
				}
			}
			...
			TGraph* gr_stds;
			for (int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){
				gr_stds = new TGraph(nSetsJustBS, nBoostdstrapped, asymmetries_000_eta_err_BS[iTag][iBaryonBin]);
				gr_stds->SetTitle("Beam Asymmetry BS errors 0/90 eta");
				//gr_stds->SetMarkerColor(iBaryonBin+1);
				//gr_stds->SetMarkerStyle(21);
				gr_stds->SetLineColor(iBaryonBin+1);
			...
			allCanvases->cd(2);
			for (int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){
				gr_stds = new TGraph(nSetsJustBS, nBoostdstrapped, asymmetries_045_eta_err_BS[iTag][iBaryonBin]);
			...
			allCanvases->cd(3);
			for (int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){
				gr_stds = new TGraph(nSetsJustBS, nBoostdstrapped, asymmetries_000_pi0_err_BS[iTag][iBaryonBin]);
			...
			allCanvases->cd(4);
			for (int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){
				gr_stds = new TGraph(nSetsJustBS, nBoostdstrapped, asymmetries_045_pi0_err_BS[iTag][iBaryonBin]);
	...
	for (int iBaryonBin=0; iBaryonBin<num_baryonBins; ++iBaryonBin){
		...
		for (int phiIndex=0; phiIndex<sizeOfOrientationList; ++phiIndex) { 
			currentIndex = iBaryonBin*sizeOfOrientationList + phiIndex;
			cout << "CURRENT INDEX: " << currentIndex << endl;
			cout << "(" << phis_orientation[phiIndex] << ")" << "Number events in teta Bin " << iBaryonBin << ": " << (double)nEventsPhiEta[0][currentIndex] << endl;
			cout << "(" << phis_orientation[phiIndex] << ")" << "Number events in tpi0 Bin " << iBaryonBin << ": " << (double)nEventsPhiPi0[0][currentIndex] << endl;
		}
	}
	cout << "\n\n" << endl;
}
