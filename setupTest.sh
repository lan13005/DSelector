...
#if $is_pi0eta 
#then 
#	sed -i "s@pi0eta=false@pi0eta=true@g" DSelector_ver20.h
#fi
...
#if $is_pi0eta
#then 
#	sed -i "s@pi0eta=true@pi0eta=false@g" DSelector_ver20.h
#fi


totalHists=$(grep '^Total\ Number' logTest.txt | cut -d " " -f 5)
availHists=$(grep 'dHist_all1DHists' DSelector_ver20.h | cut -d "[" -f 2 | cut -d "]" -f 1)
