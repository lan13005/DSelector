...
void asymmetryMC(){
	...
	int numEntries000 = phi000_eta_total[0][0]->GetEntries();
	int numEntries090 = phi090_eta_total[0][0]->GetEntries();
	...
	TF1* fit000 = new TF1("fit000","1.0-0.35*0.5*cos(2*0.01745*x)",-180,180);
	TF1* fit090 = new TF1("fit000","1.0-0.35*0.5*cos(2*0.01745*(x-90))",-180,180);
	...
	TH1F* dHist_asymmetries = new TH1F("dHist_asymmetries","dHist_asymmetries;#Sigma",100,0.4,0.6);
	TH1F* dHist_asymmetryErrors = new TH1F("dHist_asymmetryErrors","dHist_asymmetryErrors;#sigma_{#Sigma}",100,0.016,0.02);
	int maxPrint = 2;
	...
	for (int iter=0; iter<1000; ++iter){
		...
		TH1F* dHist_simulated000 = new TH1F(("dHist_simulated000"+to_string(iter)).c_str(),"dHist_simulated000; #phi degrees", 90,-180,180);
		TH1F* dHist_simulated090 = new TH1F(("dHist_simulated090"+to_string(iter)).c_str(),"dHist_simulated090; #phi degrees", 90,-180,180);
		...
		asymmetries.push_back(fit_asym->GetParameter(2));
		asymmetryErrors.push_back(abs(fit_asym->GetParError(2)));
		dHist_asymmetries->Fill(fit_asym->GetParameter(2));
		dHist_asymmetryErrors->Fill(abs(fit_asym->GetParError(2)));

		if (iter<maxPrint){
			allCanvases->Clear();
			allCanvases->Divide(1,3);
			allCanvases->cd(1);
			dHist_simulated000->Draw();
			allCanvases->cd(2);
			dHist_simulated090->Draw();
			allCanvases->cd(3);
			asymmetry000_090_eta->Draw();
			allCanvases->SaveAs(("asymmetryMC/simulatedAsymmetry_set"+to_string(iter)+".png").c_str());
		}
	}

	//allCanvases->Clear();
	//allCanvases->Divide(1,2);
	//allCanvases->cd(1);
	//dHist_asymmetries->Draw();
	//allCanvases->cd(2);
	//dHist_asymmetryErrors->Draw();
	//allCanvases->SaveAs("asymmetryMC/simulatedSigma_SigErr.png");
	dHist_asymmetries->SaveAs("asymmetryMC/Asymmetries.C");
	dHist_asymmetryErrors->SaveAs("asymmetryMC/AsymmetryErrors.C");
