...
for seedIdx in range(len(seedNums)):
	...
	for binNum in binsArray:
		...
		bin_cfg = bin_cfg_base+".cfg"
		param_init_seed = "seed"+str(seedShifted)+"-"+seedFile
		...
		# if not we will begin by randomly sampling
		...
		for j in range(maxIter):
			...
			logFileName="bin_"+str(binNum)+"-seed"+str(seedShifted)+".log"
			...
			if (num_lines>0):
				...
				#print("renaming bin_"+str(binNum)+".fit to bin_"+str(binNum)+"-seed"+str(seedNum)+".fit")
				#os.rename("bin_"+str(binNum)+".fit","bin_"+str(binNum)+"-seed"+str(seedNum)+".fit")
				break
			if (num_lines==0):
