...
void fitAsymmetryPlots3(){
	...
	static const int num_Mpi0etaBins=5;
	...
	cout << "Shape of the tensor of histograms: ["<<nTagEta<<"]["<<num_Mpi0etaBins<<"]["<<nDataSets<<"]["<<nSetsBS<<"]"<< endl;
	TH1F *phi000_eta_unscaled[nTagEta][num_Mpi0etaBins][nDataSets][nSetsBS]; // 2 for the tag and 3 for the datasets
	TH1F *phi045_eta_unscaled[nTagEta][num_Mpi0etaBins][nDataSets][nSetsBS]; 
	TH1F *phi090_eta_unscaled[nTagEta][num_Mpi0etaBins][nDataSets][nSetsBS];
	TH1F *phi135_eta_unscaled[nTagEta][num_Mpi0etaBins][nDataSets][nSetsBS];
	TH1F *phiAMO_eta_unscaled[nTagEta][num_Mpi0etaBins][nDataSets][nSetsBS];
	TH1F *phi000_pi0_unscaled[nTagEta][num_Mpi0etaBins][nDataSets][nSetsBS];
	TH1F *phi045_pi0_unscaled[nTagEta][num_Mpi0etaBins][nDataSets][nSetsBS];
	TH1F *phi090_pi0_unscaled[nTagEta][num_Mpi0etaBins][nDataSets][nSetsBS];
	TH1F *phi135_pi0_unscaled[nTagEta][num_Mpi0etaBins][nDataSets][nSetsBS];
	TH1F *phiAMO_pi0_unscaled[nTagEta][num_Mpi0etaBins][nDataSets][nSetsBS];
	// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if (nSetsBS == 1){
		for (int iSet=0; iSet<nSetsBS; ++iSet){ // we add 1 since using the full dataset takes the first element of the array
			for (int iData=0; iData <nDataSets; ++iData){
				...
				for (int iTag=0; iTag < nTagEta; ++iTag){
					for (int iBaryonBin=0; iBaryonBin<num_Mpi0etaBins; ++iBaryonBin){
						string affix = "Bin"+to_string(iBaryonBin);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_045_Mpi0eta_fastEta"+affix).c_str(), phi045_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_045_Mpi0eta_fastPi0"+affix).c_str(), phi045_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_090_Mpi0eta_fastEta"+affix).c_str(), phi090_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_090_Mpi0eta_fastPi0"+affix).c_str(), phi090_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_AMO_Mpi0eta_fastEta"+affix).c_str(), phiAMO_eta_unscaled[iTag][iBaryonBin][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_AMO_Mpi0eta_fastPi0"+affix).c_str(), phiAMO_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
						...
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_135_Mpi0eta_fastPi0"+affix).c_str(), phi135_pi0_unscaled[iTag][iBaryonBin][iData][iSet]);
					}
		...
		for ( int iData=0; iData<nDataSets; ++iData){ // the dataset we use, 2017 or the two 2018 sets
			for (int iTag=0; iTag < nTagEta; ++iTag){ // nothing or backwardPi0/Etap
				for (int iBaryonBin=0; iBaryonBin<num_Mpi0etaBins; ++iBaryonBin){ // t bins
					string name0 = "prodPlanePSphi"+tagEta[iTag]+"_045_Mpi0eta_fastEtaBin"+to_string(iBaryonBin);
					string name1 = "prodPlanePSphi"+tagPi0[iTag]+"_045_Mpi0eta_fastPi0Bin"+to_string(iBaryonBin);
					string name2 = "prodPlanePSphi"+tagEta[iTag]+"_090_Mpi0eta_fastEtaBin"+to_string(iBaryonBin);
					string name3 = "prodPlanePSphi"+tagPi0[iTag]+"_090_Mpi0eta_fastPi0Bin"+to_string(iBaryonBin);
					string name4 = "prodPlanePSphi"+tagEta[iTag]+"_AMO_Mpi0eta_fastEtaBin"+to_string(iBaryonBin);
					string name5 = "prodPlanePSphi"+tagPi0[iTag]+"_AMO_Mpi0eta_fastPi0Bin"+to_string(iBaryonBin);
					string name6 = "prodPlanePSphi"+tagEta[iTag]+"_000_Mpi0eta_fastEtaBin"+to_string(iBaryonBin);
					string name7 = "prodPlanePSphi"+tagPi0[iTag]+"_000_Mpi0eta_fastPi0Bin"+to_string(iBaryonBin);
					string name8 = "prodPlanePSphi"+tagEta[iTag]+"_135_Mpi0eta_fastEtaBin"+to_string(iBaryonBin);
					string name9 = "prodPlanePSphi"+tagPi0[iTag]+"_135_Mpi0eta_fastPi0Bin"+to_string(iBaryonBin);
	...
	// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	// --------------------------------------------------------------------------------------------------------------------
	double asymmetries_000_eta[nTagEta][nSetsBS][num_Mpi0etaBins];
	double asymmetries_045_eta[nTagEta][nSetsBS][num_Mpi0etaBins];
	double asymmetries_000_eta_err[nTagEta][nSetsBS][num_Mpi0etaBins];
	double asymmetries_045_eta_err[nTagEta][nSetsBS][num_Mpi0etaBins];
	double asymmetries_000_pi0[nTagEta][nSetsBS][num_Mpi0etaBins];
	double asymmetries_045_pi0[nTagEta][nSetsBS][num_Mpi0etaBins];
	double asymmetries_000_pi0_err[nTagEta][nSetsBS][num_Mpi0etaBins];
	double asymmetries_045_pi0_err[nTagEta][nSetsBS][num_Mpi0etaBins];
	...
	for (int iSet=0; iSet <nSetsBS; ++iSet) {
		...
		TH1F *phi000_eta_total[nTagEta][num_Mpi0etaBins];
		TH1F *phi045_eta_total[nTagEta][num_Mpi0etaBins];
		TH1F *phi090_eta_total[nTagEta][num_Mpi0etaBins];
		TH1F *phi135_eta_total[nTagEta][num_Mpi0etaBins];
		TH1F *phiAMO_eta_total[nTagEta][num_Mpi0etaBins];
		TH1F *phi000_pi0_total[nTagEta][num_Mpi0etaBins];
		TH1F *phi045_pi0_total[nTagEta][num_Mpi0etaBins];
		TH1F *phi090_pi0_total[nTagEta][num_Mpi0etaBins];
		TH1F *phi135_pi0_total[nTagEta][num_Mpi0etaBins];
		TH1F *phiAMO_pi0_total[nTagEta][num_Mpi0etaBins];
		...
		Int_t fitStatus;
		for (int iTag=0; iTag<nTagEta; ++iTag){
			for (int iBaryonBin=0; iBaryonBin<num_Mpi0etaBins; ++iBaryonBin){ //num_Mpi0etaBins
				...
				// *****************************
				mBins[iBaryonBin] = minMpi0eta+(iBaryonBin+0.5)*mBinSize;
				mBins_err[iBaryonBin] = mBinSize/2;
				...
				if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/asymmetry"+tagEta[iTag]+"_Mpi0eta_fastEtaBin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".png").c_str()); }
				...
				//if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/likelihood"+tagEta[iTag]+"_Mpi0etaBin"+to_string(iMass)+"_Mpi0eta_fastEtaBin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".root").c_str()); }
				...
				if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/asymmetry"+tagPi0[iTag]+"_Mpi0eta_fastPi0Bin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".png").c_str()); }
				...
				if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/phiYieldFits"+tagEta[iTag]+"_Mpi0eta_fastEtaBin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".png").c_str()); }
				...
				if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/phiYieldFits"+tagPi0[iTag]+"_Mpi0eta_fastPi0Bin"+to_string(iBaryonBin)+"_iSet"+to_string(iSet)+".png").c_str()); }
