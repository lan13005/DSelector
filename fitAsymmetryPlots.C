...
void fitAsymmetryPlots(){
	...
	cout << "Shape of the tensor of histograms: ["<<nTagEta<<"]["<<num_mBins<<"]["<<num_tBins<<"]["<<nDataSets<<"]["<<nSetsBS<<"]"<< endl;
	...
	if (nSetsBS == 1){
		for (int iSet=0; iSet<nSetsBS; ++iSet){ // we add 1 since using the full dataset takes the first element of the array
			for (int iData=0; iData <nDataSets; ++iData){
				string dataFileName = "/d/grid15/ln16/pi0eta/092419/degALL_"+dataSetTag[iData]+"_mEllipsePre_hists_DSelector.root";
		...
		std::vector<double> values[nTagEta][num_mBins][num_tBins][nDataSets][nHists];
		std::vector<double> weights[nTagEta][num_mBins][num_tBins][nDataSets][nHists];
			...
			for (int iTag=0; iTag < nTagEta; ++iTag){ // nothing or backwardPi0/Etap
				for (int iteta=0; iteta<num_tBins; ++iteta){ // t bins
					...
					string nameTag = "_iData"+to_string(iData)+"_iSet0"; 
        				...
        				phi045_eta_unscaled[iTag][0][iteta][iData][0] = new TH1F((name0+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi045_pi0_unscaled[iTag][0][iteta][iData][0] = new TH1F((name1+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi090_eta_unscaled[iTag][0][iteta][iData][0] = new TH1F((name2+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi090_pi0_unscaled[iTag][0][iteta][iData][0] = new TH1F((name3+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phiAMO_eta_unscaled[iTag][0][iteta][iData][0] = new TH1F((name4+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phiAMO_pi0_unscaled[iTag][0][iteta][iData][0] = new TH1F((name5+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi000_eta_unscaled[iTag][0][iteta][iData][0] = new TH1F((name6+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi000_pi0_unscaled[iTag][0][iteta][iData][0] = new TH1F((name7+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi135_eta_unscaled[iTag][0][iteta][iData][0] = new TH1F((name8+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
        				phi135_pi0_unscaled[iTag][0][iteta][iData][0] = new TH1F((name9+nameTag).c_str(), "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees", 40, -180, 180);
					...
					std::vector<TH1F*> hists = {phi045_eta_unscaled[iTag][0][iteta][iData][0],phi045_pi0_unscaled[iTag][0][iteta][iData][0]
						,phi090_eta_unscaled[iTag][0][iteta][iData][0]
						,phi090_pi0_unscaled[iTag][0][iteta][iData][0],phiAMO_eta_unscaled[iTag][0][iteta][iData][0],phiAMO_pi0_unscaled[iTag][0][iteta][iData][0]
						,phi000_eta_unscaled[iTag][0][iteta][iData][0],phi000_pi0_unscaled[iTag][0][iteta][iData][0],phi135_eta_unscaled[iTag][0][iteta][iData][0]
						,phi135_pi0_unscaled[iTag][0][iteta][iData][0]};
					...
					for ( int iHist=0; iHist<nHists; ++iHist){
						while (*(inFiles[iHist]) >> value >> weight){
							//cout << value << endl;
							values[iTag][0][iteta][iData][iHist].push_back(value);
							weights[iTag][0][iteta][iData][iHist].push_back(weight);
							hists[iHist]->Fill(value,weight);
						}
						hists[iHist]->Print();
						nevents[iHist] = values[iTag][0][iteta][iData][iHist].size();	
					...
					for (int iSet=1; iSet<nSetsBS; ++iSet){ // we add 1 since using the full dataset takes the first element of the array
						...
						string cutTag = "Cuts=ptEtaBeamAsym;#phi; Entries / 9 degrees";
        					phi045_eta_unscaled[iTag][0][iteta][iData][iSet] = new TH1F((name0+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi045_pi0_unscaled[iTag][0][iteta][iData][iSet] = new TH1F((name1+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi090_eta_unscaled[iTag][0][iteta][iData][iSet] = new TH1F((name2+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi090_pi0_unscaled[iTag][0][iteta][iData][iSet] = new TH1F((name3+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phiAMO_eta_unscaled[iTag][0][iteta][iData][iSet] = new TH1F((name4+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phiAMO_pi0_unscaled[iTag][0][iteta][iData][iSet] = new TH1F((name5+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi000_eta_unscaled[iTag][0][iteta][iData][iSet] = new TH1F((name6+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi000_pi0_unscaled[iTag][0][iteta][iData][iSet] = new TH1F((name7+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi135_eta_unscaled[iTag][0][iteta][iData][iSet] = new TH1F((name8+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
        					phi135_pi0_unscaled[iTag][0][iteta][iData][iSet] = new TH1F((name9+nameTag).c_str(), cutTag.c_str(), 40, -180, 180);
						hists = {phi045_eta_unscaled[iTag][0][iteta][iData][iSet],phi045_pi0_unscaled[iTag][0][iteta][iData][iSet],phi090_eta_unscaled[iTag][0][iteta][iData][iSet]
							,phi090_pi0_unscaled[iTag][0][iteta][iData][iSet],phiAMO_eta_unscaled[iTag][0][iteta][iData][iSet],phiAMO_pi0_unscaled[iTag][0][iteta][iData][iSet]
							,phi000_eta_unscaled[iTag][0][iteta][iData][iSet],phi000_pi0_unscaled[iTag][0][iteta][iData][iSet],phi135_eta_unscaled[iTag][0][iteta][iData][iSet]
							,phi135_pi0_unscaled[iTag][0][iteta][iData][iSet]};
						int randIdx;
						for ( int iHist=0; iHist<nHists; ++iHist){
							for (int ievent=0; ievent<nevents[iHist]; ++ievent){
								randIdx = rand() % nevents[iHist];		
								value = values[iTag][0][iteta][iData][iHist][randIdx];
								weight = weights[iTag][0][iteta][iData][iHist][randIdx];
								hists[iHist]->Fill(value, weight);
							}
						}
					}
		...
		for (int iTag=0; iTag < nTagEta; ++iTag) {
			for (int iMass=0; iMass<num_mBins; ++iMass){
				for (int iteta=0; iteta<num_tBins; ++iteta){
					...
					cout << "phi000_eta_unscaled["<<iTag<<"]["<<iMass<<"]["<<iteta<<"][0]["<<iSet<<"]" << endl;
					phi000_eta_total[iTag][iMass][iteta] = (TH1F *)phi000_eta_unscaled[iTag][iMass][iteta][0][iSet]->Clone();
					phi000_pi0_total[iTag][iMass][iteta] = (TH1F *)phi000_pi0_unscaled[iTag][iMass][iteta][0][iSet]->Clone();
					phi135_eta_total[iTag][iMass][iteta] = (TH1F *)phi135_eta_unscaled[iTag][iMass][iteta][0][iSet]->Clone();
					phi135_pi0_total[iTag][iMass][iteta] = (TH1F *)phi135_pi0_unscaled[iTag][iMass][iteta][0][iSet]->Clone();
					maximum_before = phi000_eta_total[iTag][iMass][iteta]->GetMaximum();
					cout << "-Maximum before scaling of phi000_eta_total[" << iTag << "][" << iteta << "]: " << maximum_before << endl;
					cout << "-NEntries before scaling of phi000_eta_total[" << iTag << "]" << iteta << "]: " << phi000_eta_total[iTag][iMass][iteta]->GetEntries() << endl;
					phi000_eta_total[iTag][iMass][iteta]->Scale(fluxRatios_90_0[0]);
					phi000_pi0_total[iTag][iMass][iteta]->Scale(fluxRatios_90_0[0]);
					phi135_eta_total[iTag][iMass][iteta]->Scale(fluxRatios_45_135[0]);
					phi135_pi0_total[iTag][iMass][iteta]->Scale(fluxRatios_45_135[0]);
					maximum_after = phi000_eta_total[iTag][iMass][iteta]->GetMaximum();
					cout << "-Maximum after scaling of phi000_eta_total[" << iTag << "]" << iteta << "]: " << maximum_after << endl;
					cout << "-NEntries after scaling of phi000_eta_total[" << iTag << "]" << iteta << "]: " << phi000_eta_total[iTag][iMass][iteta]->GetEntries() << endl; 
					cout << "-Scale factor vs Expected: " << maximum_after/maximum_before << " vs " << fluxRatios_90_0[0] << endl; 
					phi045_eta_total[iTag][iMass][iteta] = (TH1F *)phi045_eta_unscaled[iTag][iMass][iteta][0][iSet]->Clone();
					phi045_pi0_total[iTag][iMass][iteta] = (TH1F *)phi045_pi0_unscaled[iTag][iMass][iteta][0][iSet]->Clone();
					phi090_eta_total[iTag][iMass][iteta] = (TH1F *)phi090_eta_unscaled[iTag][iMass][iteta][0][iSet]->Clone();
					phi090_pi0_total[iTag][iMass][iteta] = (TH1F *)phi090_pi0_unscaled[iTag][iMass][iteta][0][iSet]->Clone();
					phiAMO_eta_total[iTag][iMass][iteta] = (TH1F *)phiAMO_eta_unscaled[iTag][iMass][iteta][0][iSet]->Clone();
					phiAMO_pi0_total[iTag][iMass][iteta] = (TH1F *)phiAMO_pi0_unscaled[iTag][iMass][iteta][0][iSet]->Clone();

					totalEntriesInPhi000_eta_total += phi000_eta_total[iTag][iMass][iteta]->GetEntries();
					...
					for (int iData=1; iData < nDataSets; ++iData){
						...
						totalEntriesInPhi000_eta_total += phi000_eta_unscaled[iTag][iMass][iteta][iData][iSet]->GetEntries();
						...
						phi000_eta_total[iTag][iMass][iteta]->Add(phi000_eta_unscaled[iTag][iMass][iteta][iData][iSet]);
        	        			...
        	        			phi135_pi0_total[iTag][iMass][iteta]->Add(phi135_pi0_unscaled[iTag][iMass][iteta][iData][iSet]);

						// Dont have to scale these guys so just add them
        	        			phi045_eta_total[iTag][iMass][iteta]->Add(phi045_eta_unscaled[iTag][iMass][iteta][iData][iSet]);
        	        			phi045_pi0_total[iTag][iMass][iteta]->Add(phi045_pi0_unscaled[iTag][iMass][iteta][iData][iSet]);
        	        			phi090_eta_total[iTag][iMass][iteta]->Add(phi090_eta_unscaled[iTag][iMass][iteta][iData][iSet]);
        	        			phi090_pi0_total[iTag][iMass][iteta]->Add(phi090_pi0_unscaled[iTag][iMass][iteta][iData][iSet]);
        	        			phiAMO_eta_total[iTag][iMass][iteta]->Add(phiAMO_eta_unscaled[iTag][iMass][iteta][iData][iSet]);
        	        			phiAMO_pi0_total[iTag][iMass][iteta]->Add(phiAMO_pi0_unscaled[iTag][iMass][iteta][iData][iSet]);
					}
				}
			}
		}	
		...
		for (int iMass=0; iMass<num_mBins; ++iMass){
			...
			for (int iTag=0; iTag<nTagEta; ++iTag){
				for (int iteta=0; iteta<num_tBins; ++iteta){ //num_tBins
					...
					TH1F *phi000_eta = phi000_eta_total[iTag][iMass][iteta];
					TH1F *phi045_eta = phi045_eta_total[iTag][iMass][iteta];
					TH1F *phi090_eta = phi090_eta_total[iTag][iMass][iteta];
					TH1F *phi135_eta = phi135_eta_total[iTag][iMass][iteta];
					TH1F *phiAMO_eta = phiAMO_eta_total[iTag][iMass][iteta];
					TH1F *phi000_pi0 = phi000_pi0_total[iTag][iMass][iteta];
					TH1F *phi045_pi0 = phi045_pi0_total[iTag][iMass][iteta];
					TH1F *phi090_pi0 = phi090_pi0_total[iTag][iMass][iteta];
					TH1F *phi135_pi0 = phi135_pi0_total[iTag][iMass][iteta];
					TH1F *phiAMO_pi0 = phiAMO_pi0_total[iTag][iMass][iteta];
					...
					if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/SigVst1_threshMpi0eta/asymmetry"+tagEta[iTag]+"_Mpi0etaBin"+to_string(iMass)+"_tetaBin"+to_string(iteta)+"_iSet"+to_string(iSet)+".png").c_str()); }
					...
					//if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/SiVst1_threshMpi0eta/likelihood"+tagEta[iTag]+"_Mpi0etaBin"+to_string(iMass)+"_tetaBin"+to_string(iteta)+"_iSet"+to_string(iSet)+".root").c_str()); }
					...
					if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/SigVst1_threshMpi0eta/asymmetry"+tagPi0[iTag]+"_Mpi0etaBin"+to_string(iMass)+"_tpi0Bin"+to_string(iteta)+"_iSet"+to_string(iSet)+".png").c_str()); }
					...
					if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/SigVst1_threshMpi0eta/phiYieldFits"+tagEta[iTag]+"_Mpi0etaBin"+to_string(iMass)+"_tetaBin"+to_string(iteta)+"_iSet"+to_string(iSet)+".png").c_str()); }
					...
					if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/SigVst1_threshMpi0eta/phiYieldFits"+tagPi0[iTag]+"_Mpi0etaBin"+to_string(iMass)+"_tpi0Bin"+to_string(iteta)+"_iSet"+to_string(iSet)+".png").c_str()); }
				...
				auto leg1 = new TLegend(0.7,0.15,0.9,0.35);
	                        allCanvases = new TCanvas("","",1440,900);
                                TPaveLabel* title = new TPaveLabel(0.1,0.91,0.9,0.97,("M(#pi#eta) < "+to_string(minMpi0eta+iMass*0.1)).c_str());
				...
				if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/SigVst1_threshMpi0eta/asymVst"+tag[iTag]+"_Mpi0etaBin"+to_string(iMass)+"_iSet"+to_string(iSet)+".png").c_str()); }
				...
				allCanvases->cd(2);
   				legend_pi0->Draw();
				if ( iSet < maxPrintBS ){ allCanvases->SaveAs(("asymmetryPlots/SigVst1_threshMpi0eta/PSigma_Mpi0etaBin"+to_string(iMass)+"_iSet"+to_string(iSet)+".png").c_str()); }
			}
		}
	}
	...
	if (nSetsBS != 1) {
		...
		for (int iTag=0; iTag < 1; ++iTag) { //nTagEta
			...
			allAsymsBSFile.open(("asymmetryPlots/SigVst1_threshMpi0eta/allAsymsBSFile_iTag"+to_string(iTag)+".txt").c_str(),std::ios_base::trunc);
			...
			allCanvases->SaveAs(("asymmetryPlots/SigVst1_threshMpi0eta/bootstrappedErrors_iTag"+to_string(iTag)+".png").c_str());
		}
	}
