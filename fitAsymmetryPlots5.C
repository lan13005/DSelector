...
void fitAsymmetryPlots5(){
	...
	cout << "Shape of the tensor of histograms: ["<<nTagEta<<"]["<<num_tBins<<"]["<<nDataSets<<"]["<<nSetsBS<<"]"<< endl;
	...
	if (nSetsBS == 1){
		for (int iSet=0; iSet<nSetsBS; ++iSet){ // we add 1 since using the full dataset takes the first element of the array
			for (int iData=0; iData <nDataSets; ++iData){
				string dataFileName = "/d/grid15/ln16/pi0eta/092419/degALL_data_"+dataSetTag[iData]+"_mEllipsePre_hists_DSelector.root";
				...
				for (int iTag=0; iTag < nTagEta; ++iTag){
					for (int it=0; it<num_tBins; ++it){
						string affixEta = "tightCuts_tetaBin"+to_string(it);
						string affixPi0 = "tightCuts_tpi0Bin"+to_string(it);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_045_"+affixEta).c_str(), phi045_eta_unscaled[iTag][it][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_045_"+affixPi0).c_str(), phi045_pi0_unscaled[iTag][it][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_090_"+affixEta).c_str(), phi090_eta_unscaled[iTag][it][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_090_"+affixPi0).c_str(), phi090_pi0_unscaled[iTag][it][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_AMO_"+affixEta).c_str(), phiAMO_eta_unscaled[iTag][it][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_AMO_"+affixPi0).c_str(), phiAMO_pi0_unscaled[iTag][it][iData][iSet]);
						// Need to scale these para yields by flux ratio.
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_000_"+affixEta).c_str(), phi000_eta_unscaled[iTag][it][iData][iSet]);
						cout << "phi000_eta_unscaled["<<iTag<<"]["<<it<<"]["<<iData<<"]["<<iSet<<"]" << endl;
						cout << (" -- prodPlanePSphi"+tagEta[iTag]+"_000_"+affixEta).c_str() << endl;
						cout << " -- nentries=" << phi000_eta_unscaled[iTag][it][iData][iSet]->GetEntries() << endl;
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_000_"+affixPi0).c_str(), phi000_pi0_unscaled[iTag][it][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagEta[iTag]+"_135_"+affixEta).c_str(), phi135_eta_unscaled[iTag][it][iData][iSet]);
						dataFile->GetObject(("prodPlanePSphi"+tagPi0[iTag]+"_135_"+affixPi0).c_str(), phi135_pi0_unscaled[iTag][it][iData][iSet]);
					}
				}
			}
		}
	...
	// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	// --------------------------------------------------------------------------------------------------------------------
	double asymmetries_000_eta[nTagEta][nSetsBS][num_tBins];
	double asymmetries_045_eta[nTagEta][nSetsBS][num_tBins];
	double asymmetries_000_eta_err[nTagEta][nSetsBS][num_tBins];
	double asymmetries_045_eta_err[nTagEta][nSetsBS][num_tBins];
	double asymmetries_000_pi0[nTagEta][nSetsBS][num_tBins];
	double asymmetries_045_pi0[nTagEta][nSetsBS][num_tBins];
	double asymmetries_000_pi0_err[nTagEta][nSetsBS][num_tBins];
	double asymmetries_045_pi0_err[nTagEta][nSetsBS][num_tBins];
	...
	for (int iSet=0; iSet <nSetsBS; ++iSet) {
		...
		for (int iTag=0; iTag < nTagEta; ++iTag) {
			for (int it=0; it<num_tBins; ++it){
				...
				phi000_eta_total[iTag][it] = (TH1F *)phi000_eta_unscaled[iTag][it][0][iSet]->Clone();
				phi000_pi0_total[iTag][it] = (TH1F *)phi000_pi0_unscaled[iTag][it][0][iSet]->Clone();
				phi135_eta_total[iTag][it] = (TH1F *)phi135_eta_unscaled[iTag][it][0][iSet]->Clone();
				phi135_pi0_total[iTag][it] = (TH1F *)phi135_pi0_unscaled[iTag][it][0][iSet]->Clone();
				maximum_before = phi000_eta_total[iTag][it]->GetMaximum();
				cout << "-Maximum before scaling of phi000_eta_total[" << iTag << "][" << it << "]: " << maximum_before << endl;
				cout << "-NEntries before scaling of phi000_eta_total[" << iTag << "][" << it << "]: " << phi000_eta_total[iTag][it]->GetEntries() << endl;
				phi000_eta_total[iTag][it]->Scale(fluxRatios_90_0[0]);
				phi000_pi0_total[iTag][it]->Scale(fluxRatios_90_0[0]);
				phi135_eta_total[iTag][it]->Scale(fluxRatios_45_135[0]);
				phi135_pi0_total[iTag][it]->Scale(fluxRatios_45_135[0]);
				maximum_after = phi000_eta_total[iTag][it]->GetMaximum();
				cout << "-Maximum after scaling of phi000_eta_total[" << iTag << "]" << it << "]: " << maximum_after << endl;
				cout << "-NEntries after scaling of phi000_eta_total[" << iTag << "][" << it << "]: " << phi000_eta_total[iTag][it]->GetEntries() << endl; 
				cout << "-Scale factor vs Expected: " << maximum_after/maximum_before << " vs " << fluxRatios_90_0[0] << endl; 
				phi045_eta_total[iTag][it] = (TH1F *)phi045_eta_unscaled[iTag][it][0][iSet]->Clone();
				phi045_pi0_total[iTag][it] = (TH1F *)phi045_pi0_unscaled[iTag][it][0][iSet]->Clone();
				phi090_eta_total[iTag][it] = (TH1F *)phi090_eta_unscaled[iTag][it][0][iSet]->Clone();
				phi090_pi0_total[iTag][it] = (TH1F *)phi090_pi0_unscaled[iTag][it][0][iSet]->Clone();
				phiAMO_eta_total[iTag][it] = (TH1F *)phiAMO_eta_unscaled[iTag][it][0][iSet]->Clone();
				phiAMO_pi0_total[iTag][it] = (TH1F *)phiAMO_pi0_unscaled[iTag][it][0][iSet]->Clone();

				totalEntriesInPhi000_eta_total += phi000_eta_total[iTag][it]->GetEntries();
				...
				for (int iData=1; iData < nDataSets; ++iData){
					...
					totalEntriesInPhi000_eta_total += phi000_eta_unscaled[iTag][it][iData][iSet]->GetEntries();
					...
					phi000_eta_unscaled[iTag][it][iData][iSet]->Scale(fluxRatios_90_0[iData]);
					phi000_pi0_unscaled[iTag][it][iData][iSet]->Scale(fluxRatios_90_0[iData]);
					phi135_eta_unscaled[iTag][it][iData][iSet]->Scale(fluxRatios_45_135[iData]);
					phi135_pi0_unscaled[iTag][it][iData][iSet]->Scale(fluxRatios_45_135[iData]);
					phi000_eta_total[iTag][it]->Add(phi000_eta_unscaled[iTag][it][iData][iSet]);
        	        		phi000_pi0_total[iTag][it]->Add(phi000_pi0_unscaled[iTag][it][iData][iSet]);
        	        		phi135_eta_total[iTag][it]->Add(phi135_eta_unscaled[iTag][it][iData][iSet]);
        	        		phi135_pi0_total[iTag][it]->Add(phi135_pi0_unscaled[iTag][it][iData][iSet]);

					// Dont have to scale these guys so just add them
        	        		phi045_eta_total[iTag][it]->Add(phi045_eta_unscaled[iTag][it][iData][iSet]);
        	        		phi045_pi0_total[iTag][it]->Add(phi045_pi0_unscaled[iTag][it][iData][iSet]);
        	        		phi090_eta_total[iTag][it]->Add(phi090_eta_unscaled[iTag][it][iData][iSet]);
        	        		phi090_pi0_total[iTag][it]->Add(phi090_pi0_unscaled[iTag][it][iData][iSet]);
        	        		phiAMO_eta_total[iTag][it]->Add(phiAMO_eta_unscaled[iTag][it][iData][iSet]);
        	        		phiAMO_pi0_total[iTag][it]->Add(phiAMO_pi0_unscaled[iTag][it][iData][iSet]);
				}
		...
		for (int iTag=0; iTag<nTagEta; ++iTag){
			 for (int it=0; it<num_tBins; ++it){
			 	...
			 	TH1F *phi000_eta = phi000_eta_total[iTag][it];
			 	TH1F *phi045_eta = phi045_eta_total[iTag][it];
			 	TH1F *phi090_eta = phi090_eta_total[iTag][it];
			 	TH1F *phi135_eta = phi135_eta_total[iTag][it];
			 	TH1F *phiAMO_eta = phiAMO_eta_total[iTag][it];
			 	TH1F *phi000_pi0 = phi000_pi0_total[iTag][it];
			 	TH1F *phi045_pi0 = phi045_pi0_total[iTag][it];
			 	TH1F *phi090_pi0 = phi090_pi0_total[iTag][it];
			 	TH1F *phi135_pi0 = phi135_pi0_total[iTag][it];
			 	TH1F *phiAMO_pi0 = phiAMO_pi0_total[iTag][it];
			 	...
			 	asymmetries_000_eta[iTag][iSet][it] = fit_asym->GetParameter(2);
			 	asymmetries_000_eta_err[iTag][iSet][it] = fit_asym->GetParError(2);
			 	...
			 	asymmetries_045_eta[iTag][iSet][it] = fit_asym->GetParameter(2);
			 	asymmetries_045_eta_err[iTag][iSet][it] = fit_asym->GetParError(2);
			 	...
			 	asymmetries_000_pi0[iTag][iSet][it] = fit_asym->GetParameter(2);
			 	asymmetries_000_pi0_err[iTag][iSet][it] = fit_asym->GetParError(2);
			 	...
			 	asymmetries_045_pi0[iTag][iSet][it] = fit_asym->GetParameter(2);
			 	asymmetries_045_pi0_err[iTag][iSet][it] = fit_asym->GetParError(2);
			 ...
			 auto gr_000 = new TGraphErrors(num_tBins,tBins,asymmetries_000_eta[iTag][iSet],tBins_err,asymmetries_000_eta_err[iTag][iSet]);
			 ...
			 allCanvases->cd(2);
			 gPad->SetBottomMargin(0.15);
			 auto gr_045 = new TGraphErrors(num_tBins,tBins,asymmetries_045_eta[iTag][iSet],tBins_err,asymmetries_045_eta_err[iTag][iSet]);
	...
	for (int phiIndex=0; phiIndex<sizeOfOrientationList; ++phiIndex) { 
		cout << "(" << phis_orientation[phiIndex] << ")" << "Number events: " << (double)nEventsPhiEta[0][phiIndex] << endl;
		cout << "(" << phis_orientation[phiIndex] << ")" << "Number events: " << (double)nEventsPhiPi0[0][phiIndex] << endl;
	}
